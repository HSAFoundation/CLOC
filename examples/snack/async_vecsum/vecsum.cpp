...
void sumrem(int N, INT_TYPE*invec, INT_TYPE * result){
   INT_TYPE sum=0;
   for(int i=0; i<N; i++) sum +=invec[i];
   *result=sum;
}
...
int main(int argc, char **argv) {
   ...
   int vecrem;
   invec = (INT_TYPE*) malloc_global(VECLEN*sizeof(INT_TYPE));
   outvec1 = (INT_TYPE*) malloc_global(vec1_len*sizeof(INT_TYPE));
   outvec2 = (INT_TYPE*) malloc_global(vec2_len*sizeof(INT_TYPE));
   
   /* Initialize */
   for(int i=0; i<VECLEN; i++)  invec[i]= (INT_TYPE) 2; 
   ...
   for(int i=0; i<vec1_len-1; i++) 
      sum8192Kernel(&invec[i*SUMS_PER_WKG], &outvec1[i], lparm1); 
   vecrem = ((VECLEN+SUMS_PER_WKG-1) % SUMS_PER_WKG)+1;
   int base=(vec1_len-1)*SUMS_PER_WKG;
   sumrem(vecrem, &invec[base], &outvec1[vec1_len-1]);
   for(int i=0; i<vec2_len-1; i++)  
      sum8192Kernel(&outvec1[i*SUMS_PER_WKG],&outvec2[i],lparm1); 
   vecrem = ((vec1_len+SUMS_PER_WKG-1) % SUMS_PER_WKG)+1;
   outvec2[vec2_len-1]=0;
   base=(vec2_len-1)*SUMS_PER_WKG;
   sumrem(vecrem, &outvec1[base], &outvec2[vec2_len-1]);
   result = 0;
   ...
   sumrem( ((VECLEN+SUMS_PER_WKG-1) % SUMS_PER_WKG)+1, &invec[(vec1_len-1)*SUMS_PER_WKG],&outvec1[vec1_len-1]);
   ...
   sumrem( ((vec1_len+SUMS_PER_WKG-1) % SUMS_PER_WKG)+1, &outvec1[(vec2_len-1)*SUMS_PER_WKG],&outvec2[vec2_len-1]); 

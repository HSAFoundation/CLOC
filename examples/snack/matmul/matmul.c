/*
   ...
   It calls two different kernels, simple_sgemm_tt and  tiled_sgemm_tt
...
/*  This header file is generated by cloc -c matmulKernels.cl */
...
int main(int argc, char* argv[]){
   ...
   Cref.elements = (float*)malloc_global(Cref.stride * Cref.hpad * sizeof(float));
...
*/
   ...
   CPU_sgemm_tn(A.height,Bt.width,Bt.height,1.0,A.elements,A.stride,Bt.elements,Bt.stride,1.0,Cref.elements,Cref.stride); 
   clock_gettime(CLOCK_PROCESS_CPUTIME_ID,&end_time);
   nanosecs = get_nanosecs(start_time,end_time);
   gflopsCPU= (float) total_ops / (float) nanosecs ;

   printf("Matrix Cref: %d by %d  after CPU sgemm_tt \n",Cref.height,Cref.width);
   for(i = 0; i < MIN(10, Cref.height); i++){
      for(j = 0; j < MIN(10, Cref.width); j++)
         printf("%6.0f ", Cref.elements[i*Cref.stride + j]);
      printf("\n");
   }
   printf("\n");
   ...
   printf("Verify after simple_sgemm_tn:%s\n", verify(C.elements, Cref.elements, C.height * C.stride) ? "Data matches" : "Data mismatch");
   ...
   printf("Verify after tiled_sgemm_tt:%s\n", verify(C.elements, Cref.elements, C.height * C.stride) ? "Data matches" : "Data mismatch");
   /* zero C */
   for(i = 0; i < C.hpad; i++)
      for(j = 0; j < C.stride; j++)
         C.elements[i*C.stride+j] = 0.0;
